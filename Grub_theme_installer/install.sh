...
if [[ "$GRUB_TIMEOUT_STYLE" != "menu" ]]; then
    ...
    read -p "Do you want to change it to 'menu'? (y/n): " change_timeout_style
    if [[ $change_timeout_style == "y" ]]; then
        sed -i "s/^GRUB_TIMEOUT_STYLE=.*/GRUB_TIMEOUT_STYLE=\"menu\"/" "$GRUB_CONFIG"
        echo "GRUB_TIMEOUT_STYLE has been updated to 'menu'."
    fi
fi
...
if [[ "$GRUB_GFXMODE" != "auto" ]]; then
    ...
    read -p "Do you want to change it to 'auto'? (y/n): " change_gfxmode
    if [[ $change_gfxmode == "y" ]]; then
        sed -i "s/^GRUB_GFXMODE=.*/GRUB_GFXMODE=\"auto\"/" "$GRUB_CONFIG"
        echo "GRUB_GFXMODE has been updated to 'auto'."
    fi
fi
